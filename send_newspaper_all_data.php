...
try {
 ...
 $conn_n_cour->setAttribute( PDO::MYSQL_ATTR_USE_BUFFERED_QUERY, true );
    ...
    $stmt = $conn_n_cour->prepare("INSERT INTO tblIssue(transaction_id,transid,vername,SentDate,sent,agcpy,user,sent_mode,news_rate,news_price,courier_description)
    VALUES (:transaction_id,:transid,:vername,:SentDate,:sent,:agcpy,:user,:sent_mode,:news_rate,:news_price,:courier_description)");
...
if(($type=="Agents")||($type=="All")){
    ...
    try {
     ...
     $conn_n_ag->setAttribute( PDO::MYSQL_ATTR_USE_BUFFERED_QUERY, true );
        ...
        // prepare sql and bind parameters
        $stmt = $conn_n_ag->prepare("INSERT INTO tblIssue(transaction_id,transid,vername,SentDate,sent,agcpy,user,sent_mode ,news_rate,news_price,courier_description)
        VALUES (:transaction_id,:transid,:vername,:SentDate,:sent,:agcpy,:user,:sent_mode ,:news_rate,:news_price,:courier_description)");
        $stmt->bindParam(':transaction_id', $transaction_id);
        $stmt->bindParam(':transid', $transid);
        $stmt->bindParam(':vername', $vername);
    ...
    try {
     ...
     $conn->setAttribute( PDO::MYSQL_ATTR_USE_BUFFERED_QUERY, true );
        ...
        // prepare sql and bind parameters
        $stmt = $conn->prepare("INSERT INTO tblIssue(transaction_id,transid,vername,SentDate,sent,agcpy,user,sent_mode)
        VALUES (:transaction_id,:transid,:vername,:SentDate,:sent,:agcpy,:user,:sent_mode)");
        $stmt->bindParam(':transaction_id', $transaction_id);
        $stmt->bindParam(':transid', $transid);
        $stmt->bindParam(':vername', $vername);
        $stmt->bindParam(':SentDate', $SentDate);
...
// {subscriber balance value update
    ...
    */
    ...
    $sub_status="PENDING";
        $stmt->bindParam(':ID_EN',intval($row['ID_EN']));
           
        $stmt001->bindParam(':ID_EN',intval($row['ID_EN']));
